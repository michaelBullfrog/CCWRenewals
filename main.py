...
MANUAL_TRACKER_FILE_PATH = os.getenv("MANUAL_TRACKER_FILE_PATH")
...
def download_file_from_onedrive(token, onedrive_file_path, local_file_path):
    download_url = (
        f"https://graph.microsoft.com/v1.0/users/{ONEDRIVE_USER_ID}"
        f"/drive/root:/{onedrive_file_path}:/content"
    )
    ...
    print(f"Download {onedrive_file_path} status:", response.status_code)
    if response.status_code != 200:
        print(response.text)
...
def upload_xlsx_to_onedrive(token, local_file_path, onedrive_file_path):
    upload_url = (
        f"https://graph.microsoft.com/v1.0/users/{ONEDRIVE_USER_ID}"
        f"/drive/root:/{onedrive_file_path}:/content"
    )
    ...
    print(f"Upload {onedrive_file_path} status:", response.status_code)
    if response.status_code not in (200, 201):
        print(response.text)

    response.raise_for_status()
    result = response.json()
    print("Uploaded Manual Tracker:", result.get("name"))
    print("Manual Tracker URL:", result.get("webUrl"))
...
def update_manual_tracker_from_cisco(df, token):
    """
    Updates the Manual Tracker XLSX in OneDrive.
    Adds missing Subscription IDs from Cisco and updates End Customer Name.
    Preserves manual sales fields like Quoted, Signed, Sales Rep, Notes, etc.
    """

    print("\nUpdating Manual Tracker XLSX...")

    local_tracker_file = OUTPUT_FOLDER / "manual_tracker.xlsx"

    download_file_from_onedrive(
        token=token,
        onedrive_file_path=MANUAL_TRACKER_FILE_PATH,
        local_file_path=local_tracker_file,
    )

    try:
        manual_df = pd.read_excel(local_tracker_file)
    except Exception as e:
        raise RuntimeError(
            "Failed to read Manual Tracker XLSX. Make sure it is a valid .xlsx file "
            "and not a CSV renamed to .xlsx."
        ) from e
    ...
    manual_df["Subscription ID"] = manual_df["Subscription ID"].fillna("").astype(str).str.strip()
    existing_ids = set(manual_df["Subscription ID"])
    ...
    cisco_lookup["Subscription ID"] = cisco_lookup["Subscription ID"].fillna("").astype(str).str.strip()
    cisco_lookup["End Customer Name"] = cisco_lookup["End Customer Name"].fillna("").astype(str).str.strip()
    ...
    for _, row in cisco_lookup.iterrows():
        ...
        if sub_id and sub_id not in existing_ids:
            new_rows.append({
                ...
                "End Customer Name": row.get("End Customer Name", ""),
                "Quoted": "No",
                "Quote Date": "",
                "Signed": "No",
                "Signed Date": "",
    ...
    if new_rows:
        print(f"Manual Tracker rows added: {len(new_rows)}")
        manual_df = pd.concat([manual_df, pd.DataFrame(new_rows)], ignore_index=True)
    else:
        print("Manual Tracker rows added: 0")

    name_map = dict(zip(cisco_lookup["Subscription ID"], cisco_lookup["End Customer Name"]))
    manual_df["End Customer Name"] = manual_df.apply(
        lambda row: name_map.get(str(row["Subscription ID"]).strip(), row["End Customer Name"]),
        axis=1,
    )

    extra_columns = [col for col in manual_df.columns if col not in required_columns]
    manual_df = manual_df[required_columns + extra_columns]

    manual_df.to_excel(local_tracker_file, index=False)

    upload_xlsx_to_onedrive(
        token=token,
        local_file_path=local_tracker_file,
        onedrive_file_path=MANUAL_TRACKER_FILE_PATH,
    )

    print("Manual Tracker XLSX updated successfully.")
...
def main():
    ...
    print(f"Manual Tracker OneDrive path: {MANUAL_TRACKER_FILE_PATH}")
    ...
    update_manual_tracker_from_cisco(df, graph_token)
