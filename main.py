...
# =========================
# MICROSOFT GRAPH / ONEDRIVE / SHAREPOINT LIST
# =========================
...
def upload_file_to_onedrive(local_file_path, token):
    upload_url = (
...
def graph_get_all(token, url):
    ...
    while next_url:
        ...
        all_values.extend(payload.get("value", []))
        next_url = payload.get("@odata.nextLink")

    return all_values


def get_existing_sharepoint_tracker_items(token):
    url = (
        f"https://graph.microsoft.com/v1.0/sites/{SHAREPOINT_SITE_ID}"
        f"/lists/{SHAREPOINT_LIST_ID}/items?$expand=fields&$top=200"
    )

    items = graph_get_all(token, url)
    existing = {}

    sub_field = SHAREPOINT_FIELDS["subscription_id"]

    for item in items:
        fields = item.get("fields", {}) or {}
        subscription_id = str(fields.get(sub_field, "")).strip()
        if subscription_id:
    ...
    print(f"SharePoint List existing tracked subscriptions: {len(existing)}")
    return existing


def create_sharepoint_tracker_item(token, fields_payload):
    url = (
        f"https://graph.microsoft.com/v1.0/sites/{SHAREPOINT_SITE_ID}"
    ...
    response = requests.post(url, headers=headers, json={"fields": fields_payload}, timeout=120)

    if response.status_code not in (200, 201):
        print("SharePoint create item failed:", response.status_code)
        print(response.text)
...
def patch_sharepoint_tracker_item_fields(token, item_id, fields_payload):
    url = (
        f"https://graph.microsoft.com/v1.0/sites/{SHAREPOINT_SITE_ID}"
    ...
    response = requests.patch(url, headers=headers, json=fields_payload, timeout=120)

    if response.status_code not in (200, 204):
        print("SharePoint update item failed:", response.status_code)
        print(response.text)

    response.raise_for_status()


def sync_sharepoint_tracker_from_cisco(df, token):
    """
    Adds missing Cisco subscriptions to the SharePoint List tracker.

    This preserves manual tracking fields. Existing rows are not overwritten except
    for End Customer Name when the Cisco value is present and different.
    """

    print("\nSyncing Cisco subscriptions to SharePoint List tracker...")

    if df.empty:
        print("SharePoint tracker sync skipped because Cisco dataframe is empty.")
        return

    existing = get_existing_sharepoint_tracker_items(token)
    ...
    for _, row in cisco_lookup.iterrows():
        ...
        end_customer_name = row["End Customer Name"]
        ...
        if subscription_id not in existing:
            fields_payload = {
                SHAREPOINT_FIELDS["title"]: subscription_id,
                SHAREPOINT_FIELDS["subscription_id"]: subscription_id,
                SHAREPOINT_FIELDS["end_customer_name"]: end_customer_name,
                SHAREPOINT_FIELDS["quoted"]: "No",
                SHAREPOINT_FIELDS["signed"]: "No",
                SHAREPOINT_FIELDS["ordered"]: "No",
            }

            create_sharepoint_tracker_item(token, fields_payload)
            created_count += 1

            if created_count % 25 == 0:
                print(f"SharePoint tracker rows created so far: {created_count}")

        else:
            item = existing[subscription_id]
            item_fields = item.get("fields", {}) or {}
            current_name = str(item_fields.get(SHAREPOINT_FIELDS["end_customer_name"], "")).strip()

            if end_customer_name and end_customer_name != current_name:
                patch_sharepoint_tracker_item_fields(
                    token=token,
    ...
    print(f"SharePoint tracker rows created: {created_count}")
    print(f"SharePoint tracker customer names updated: {updated_name_count}")
    print(f"SharePoint tracker rows skipped: {skipped_count}")
...
def main():
    ...
    upload_file_to_onedrive(MASTER_CSV, graph_token)

    sync_sharepoint_tracker_from_cisco(df, graph_token)
