...
# Adjust if needed
MASTER_START_DATE = "2020-01-01"

# 15-day windows worked better than 30 because one US window hit the 100 cap.
WINDOW_DAYS = 15

# CCW showed 25 / 50 / 100, so we use 100.
PAGE_SIZE = 100

# Earlier paging repeated results, so we keep page 1 and split windows instead.
...
def run_graphql(token, query, client_id):
    headers = {
        ...
        # Cisco CAPI / Modern Commerce may expect one of these.
        "client_id": client_id,
        "Client-Id": client_id,
...
def build_search_subscription_query(from_date, to_date, page):
    """
    No status filter.
    No Bill-To / Disti filter.

    This pulls whatever Cisco allows the credential set to see.
    """
...
def search_subscriptions_for_window(token, client_id, market, from_date, to_date):
    ...
    for page in range(1, MAX_PAGES_PER_WINDOW + 1):
        ...
        query = build_search_subscription_query(
            from_date=from_date,
            to_date=to_date,
            page=page
        )
        ...
        # If we hit the cap, warn so we know the window may need smaller chunks.
...
def add_dashboard_fields(df):
    ...
    # Prefer Renewal Date. Fall back to End Date.
...
def main():
    ...
    for market_config in MARKETS:
        ...
        market_rows = flatten_subscription_search_results(
    ...
    # Deduping by Market + Subscription ID.
